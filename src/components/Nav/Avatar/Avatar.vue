<template>
    <div class="flex flex-col items-center" v-if="user">
        ...
        <h3 class="font-medium text-white lg:text-gray-700 text-sm"> {{ user.name }}</h3>
        <h3 class="font-medium text-white lg:text-gray-600 text-xs">Register Date: {{ user.registerDate }}</h3>
    </div>
</template>
<script>
